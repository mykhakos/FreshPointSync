...
class ProductUpdateEventPublisher:
    ...
    def __init__(self) -> None:
        ...
        self.sync_subscribers: dict[DiffType, SyncHandlerList] = {}
        self.async_subscribers: dict[DiffType, AsyncHandlerList] = {}
    ...
    @staticmethod
    def _validate_event(
        event: Union[DiffType, Iterable[DiffType], None],
    ) -> List[DiffType]:
        """Validate the provided event(s) and return a list of event objects.

        Args:
            event (Union[DiffType, Iterable[DiffType], None], optional):
        ...
        Returns:
            list[DiffType]: A list of validated event objects.
        """
        if event is None:
            return list(DiffType)
        if isinstance(event, DiffType):
            return [event]
        if isinstance(event, Iterable) and not isinstance(event, str):  # type: ignore
            return [DiffType(e) for e in event]
        raise TypeError(
            f'Event must be a DiffType or an iterable of '
            f'DiffType, got {type(event).__name__} instead.'
        )

    def subscribe(
        self,
        handler: Handler,
        event: Union[DiffType, Iterable[DiffType], None] = None,
        run_safe: bool = True,
    ...
    def get_context(
        ...
        **kwargs: Any,
        ...
        Args:
            event (DiffType): The type of product update event
                to be broadcasted.
